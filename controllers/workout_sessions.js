...
const db = () => {
    return mongodb.getDb().db('FitnessProject').collection('workout-sessions')
};
...
const getAllData = async (req, res, next) => { 
    try {
        ...
        .catch((err) => {
            res.status(500).json({
                message: err.message || 'An error occured while retrieving the workouts from the database.'
            });
        });
    } catch (err) {
...
const getOneFromData = async (req, res, next) => {
    try {
        ...
        .catch((err) => {
            res.status(500).json({
                message: err.message || 'An error occured while retrieving the workout from the database.'
            });
        });
    } catch (err) {
        res.status(500).json(err);
...
const create = async (req, res, next) => {
    try {
        ...
        const result = await db().insertOne(newWorkoutSession);
